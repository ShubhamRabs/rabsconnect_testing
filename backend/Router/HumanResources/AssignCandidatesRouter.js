...
const AssignCandidatesRouter = express.Router();

AssignCandidatesRouter.get("/get-assign-candidate-table-data", (req, res) => {
...
AssignCandidatesRouter.post(
  "/get-assign-candidate-table-data-count",
  (req, res) => {
    const user_role = req.session.user[0].urole;
    const user_id = req.session.user[0].u_id;
    if (
      ...
      pool.getConnection(function (error, connection) {
        if (error) throw error;
        connection.query(
          "SELECT COUNT(`c_id`) AS totalCount FROM `crm_candidate_details` WHERE assign_by != ? AND assign_to != ?",
          ["0", "0"],
          (err, result) => {
            if (err) {
              console.log("error");
...
// for ES5
module.exports = AssignCandidatesRouter;
