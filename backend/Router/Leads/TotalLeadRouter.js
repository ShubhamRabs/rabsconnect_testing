...
const TotalLeadRouter = express.Router();
...
TotalLeadRouter.get("/get-total-lead-table-data", (req, res) => {
...
TotalLeadRouter.post("/get-total-lead-table-data-count", (req, res) => {
  const user_role = req.session.user[0].urole;
  const user_id = req.session.user[0].u_id;
  ...
  if (user_role === "Master" || user_role === "Admin") {
    pool.getConnection(function (error, connection) {
      ...
      connection.query(
        "SELECT COUNT(lpd.l_id) AS totalCount FROM crm_lead_primary_details AS lpd WHERE COALESCE(lpd.identity, '') != 'imported'",
        (err, result) => {
          if (err) {
            console.log("error");
          } else {
    ...
    pool.getConnection(function (error, connection) {
      if (error) throw error;
      connection.query(
        "SELECT COUNT(DISTINCT lp.l_id) AS totalCount FROM crm_lead_req_details AS lrd JOIN crm_lead_primary_details AS lp ON lp.l_id = lrd.l_id WHERE lrd.assignto_id = ? AND COALESCE(lp.identity, '') != 'imported'",
        [user_id],
        (err, result) => {
          if (err) {
            console.log(err);
...
// for ES5
module.exports = TotalLeadRouter;
