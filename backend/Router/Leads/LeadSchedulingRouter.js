  ...
  for (const cityName of cities) {
    ...
    const result = await new Promise((resolve, reject) => {
      pool.getConnection(function (error, connection) {
        ...
        connection.query(
          "SELECT DISTINCT locality FROM crm_lead_primary_details WHERE city = ? AND assign_status = ?",
          [cityName,''],
          (err, result) => {
            connection.release();
            if (err) {
...
async function getAllServiceTypes(sources, ldate_from, ldate_to) {
  ...
  if (ldate_from && ldate_to) {
    query = "SELECT DISTINCT service_type FROM crm_lead_primary_details WHERE source = ? AND (DATE(create_dt) >= ? AND DATE(create_dt) <= ?)";
  }
...
async function checkScheduler(
  ...
  if (
    ...
    if (!ldate_from || !ldate_to) {
      query =
        "SELECT * FROM crm_lead_scheduling WHERE source = '' AND service_type = '' AND pname = '' AND form_name = '' AND city = '' AND locality = ''";
    }
    else {
      query =
        "SELECT * FROM crm_lead_scheduling WHERE (ldate_from BETWEEN ? AND ? OR ldate_to BETWEEN ? AND ?) OR (ldate_from = '0000-00-00' AND ldate_to = '0000-00-00') AND source = '' AND service_type = '' AND pname = '' AND form_name = '' AND city = '' AND locality = ''";
      queryParams.push(ldate_from, ldate_to, ldate_from, ldate_to);
    }
  } else {
    if (!ldate_from || !ldate_to) {
      query =
        "SELECT * FROM crm_lead_scheduling WHERE 1 ";
    } else {
      query =
        "SELECT * FROM crm_lead_scheduling WHERE (ldate_from BETWEEN ? AND ? OR ldate_to BETWEEN ? AND ?) OR (ldate_from = '0000-00-00' AND ldate_to = '0000-00-00') ";
    ...
    if (source && source.length > 0) {
      ...
      for (let i = 0; i < source.length; i++) {
        query += "OR source LIKE ? ";
        queryParams.push(`%${source[i]}%`);
      }
      query += ") ";
    } 
    // else {
    ...
    if (service_type && service_type.length > 0) {
      ...
      for (let i = 0; i < service_type.length; i++) {
        query += "OR service_type LIKE ? ";
        queryParams.push(`%${service_type[i]}%`);
      }
      query += ") ";
    ...
    if (form_name && form_name.length > 0) {
      query += "AND (form_name = '' ";
      for (let i = 0; i < form_name.length; i++) {
        query += "OR form_name LIKE ? ";
        queryParams.push(`%${form_name[i]}%`);
      }
      query += ") ";
      // query += "AND (pname = '' OR pname IN (?)) ";
      // queryParams.push(pname);
    ...
    if (locality && locality.length > 0) {
      ...
      for (let i = 0; i < locality.length; i++) {
        query += "OR locality LIKE ? ";
        queryParams.push(`%${locality[i]}%`);
      }
      query += ") ";
...
LeadSchedulingRouter.get("/get-lead-scheduler-details", async (req, res) => {
  ...
  pool.getConnection(function (error, connection) {
    ...
    connection.query(
      "SELECT DISTINCT source, service_type, pname, city, locality,form_name FROM crm_lead_primary_details WHERE assign_status = ''",
      async (err, result) => {
        if (err) {
          console.log(err);
        } else {
          for (let i = 0; i < result.length; i++) {
            ...
            if (result[i].form_name) {
              form_name.add(result[i].form_name.trim());
            }
            if (result[i].city) {
...
LeadSchedulingRouter.post("/get-lead-scheduler-details-by-date", async (req, res) => {
  ...
  pool.getConnection(function (error, connection) {
    if (error) throw error;
    connection.query(
      "SELECT DISTINCT source FROM crm_lead_primary_details WHERE source != '' AND DATE(create_dt) >= ? AND DATE(create_dt) <= ? AND assign_status = ?", [ldate_from, ldate_to, ''],
      async (err, result) => {
        if (err) {
          console.log(err);
        } else {
...
LeadSchedulingRouter.post("/get-lead-scheduler-details-by-project-name", async (req, res) => {
  ...
  let queryParams = [];
  let query = "SELECT DISTINCT pname FROM crm_lead_primary_details WHERE pname != '' AND assign_status = '' ";
  ...
  if (ldate_from && ldate_to) {
    query += " AND (DATE(create_dt) >= ? AND DATE(create_dt) <= ?)";
...
LeadSchedulingRouter.post("/get-lead-scheduler-details-by-form-name", async (req, res) => {
  const ldate_from = req.body.data.ldate_from;
  const ldate_to = req.body.data.ldate_to;
  const source = req.body.data.source || [];
  const service_type = req.body.data.service_type || [];
  ...
  let queryParams = [];
  let query = "SELECT DISTINCT form_name FROM crm_lead_primary_details WHERE form_name != '' AND assign_status = '' ";
  if (source.length === 0 && source.includes("Facebook") && pname.length === 0 (!ldate_from && !ldate_to)) {
    res.send("select data");
    return;
  }
  // if (source.length > 0) {
...
LeadSchedulingRouter.post("/get-lead-scheduler-details-by-city-name", async (req, res) => {
  ...
  const pname = req.body.data.pname || [];
  let queryParams = [];
  let query = "SELECT DISTINCT city FROM crm_lead_primary_details WHERE city != '' AND assign_status = '' ";
...
LeadSchedulingRouter.post("/get-lead-scheduler-details-by-locality-name", async (req, res) => {
  ...
  let queryParams = [];
  let query = "SELECT DISTINCT locality FROM crm_lead_primary_details WHERE locality != '' AND assign_status = '' ";
...
LeadSchedulingRouter.post("/add-lead-scheduling", async (req, res) => {
  ...
  if (data.form_name.length > 0) {
    form_name = convertArrayToString(data.form_name);
  }
  ...
  pool.getConnection(function (error, connection) {
    if (error) throw error;
    connection.query(
      "INSERT INTO crm_lead_scheduling (schedule_type,created_by,assign_to,ldate_from,ldate_to,source,service_type,pname,form_name,city,locality,create_dt,update_dt,no_of_leads,status) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)",
      [
        data.schedule_type,
        u_id,
        assign_to,
