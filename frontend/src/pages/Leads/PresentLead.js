...
const PresentLead = ({ dispatch, myglobalData }) => {
  // Dynamic breadcrumb values for the page
  const breadcrumbValues = {
    pageName: "Today's Followup", // Displayed page name in the breadcrumb
  ...
  return (
    <LeadHandler
      ...
      tableDataFunction={GetPresentLeadsTableData} // Function to fetch table data
      tableDataCountFunction={PresentLeadTableDataCount()} // Function to fetch table data count
      FetchCounting={true} // Enable fetching and displaying data count
      breadcrumbValues={breadcrumbValues} // Dynamic breadcrumb values
      useQueryKey={["PresentLeadTableData", "PresentLeadTableCount"]} // Unique keys for the useQuery hooks
      myglobalData={myglobalData} // Additional global data
    />
  );
};

export default PresentLead;
